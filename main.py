...
from auth import verify_domain_id, verify_api_key_and_get_domain, validate_api_key
from digitalocean_client import DigitalOceanGradientClient
from memori_client import call_memori_chat
...
async def create_agent(
    ...
    Args:
        ...
        domain_api_key: Optional domain-specific API key for Memori
...
async def ask(
    ...
    domain_info: dict = Depends(verify_domain_id),
):
    """
    ...
    # Call Memori for conversation context (optional)
    # Use domain-specific API key from registered_domains table
    memori_context = None
    domain_memori_key = domain_info.get("api_key")
    if domain_memori_key:
        try:
            memori_response = await call_memori_chat(
                content=request.question,
                user_id=request.user_id,
                session_id=request.session_id,
                api_key=domain_memori_key,
            )
            if memori_response.get("success"):
                memori_context = memori_response.get("context", "")
                print(f"DEBUG: Memori context retrieved: {len(memori_context)} chars")
        except Exception as e:
            print(f"WARNING: Memori call failed: {e}")
    else:
        print("DEBUG: No Memori API key configured for this domain")

    # Enhance question with Memori context and user/session info
    enhanced_question = request.question
    if memori_context:
        enhanced_question = f"""[User: {request.user_id}, Session: {request.session_id}]
[Context from previous conversations: {memori_context}]
...
User question: {request.question}"""
    ...
    chat_endpoint = f"{agent_url}/api/v1/chat/completions"
    print(f"Agent access key: {agent_access_key}")
    print(f"DEBUG: Calling agent chat completions at {chat_endpoint}")
    ...
    try:
        ...
        if not choices:
            raise HTTPException(
                status_code=500, detail="No response choices from agent"
            )

        message_content = choices[0].get("message", {}).get("content", "")

        print(f"DEBUG: Agent response received: {len(message_content)} chars")
        ...
        await save_conversation_to_db(
            request.session_id, request.user_id, "assistant", message_content
        )
        ...
        print(f"ERROR: Agent API error: {e.response.status_code} - {error_detail}")
        raise HTTPException(
            status_code=500,
            detail=f"Agent chat completions API error: {e.response.status_code}",
        )
    except Exception as e:
        import traceback

        print(f"ERROR: Failed to call agent chat completions: {e}")
...
async def upload_file_to_knowledge(
    ...
    auth_info: dict = Depends(verify_api_key_and_get_domain),
):
    """
    ...
    """
    try:
        # Get website URL from auth info
        website_url = auth_info["website_url"]
        website_key = get_website_key(website_url)
...
async def get_supported_file_types(
    ...
    return {
        ...
        "registered_domain": auth_info["domain_name"],
        "website_url": auth_info["website_url"],
    }
...
async def register_domain(
    ...
    """Register a new domain (protected endpoint).

    This endpoint must be called from a trusted backend. It validates the caller's API key via the
    existing verify_api_key dependency, stores the domain name and associated API key in the
    registered_domains table, and returns the new domain's ID.

    The agent is created immediately but deployment happens in the background (takes 1-2 minutes).

    **Authentication**: Click the "Authorize" button (🔓) at the top right to provide your API key.
    """
    try:
        ...
        if not credentials:
            raise HTTPException(
                status_code=401,
                detail="Missing Authorization header. Click 'Authorize' button to provide your API key.",
        ...
        try:
            ...
            print(
                f"DEBUG: API key validation successful for user: {validation.user.email}"
            )
        except Exception as e:
        ...
        try:
            # Check if API key is already used for another domain
            existing_api_key_row = await conn.fetchrow(
                "SELECT domain_name FROM registered_domains WHERE api_key = $1", api_key
            )

            if existing_api_key_row:
                return JSONResponse(
                    status_code=409,
                    content={
                        "message": "API key already used for another domain",
                        "existing_domain": existing_api_key_row["domain_name"],
                    },
                )

            try:
                row = await conn.fetchrow(
                    "INSERT INTO registered_domains (domain_name, api_key, created_at) VALUES ($1, $2, $3) RETURNING id",
                    domain_name,
                    api_key,
                    datetime.now(),
                )
